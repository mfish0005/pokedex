...
export class Button {
  ...
  get buttonClasses(): string {
    ...
    // Base display and alignment classes from helpers
    classes.push('is-inline-flex', 'is-flex-centered');
    
    // Typography helpers
    classes.push('has-text-weight-normal');
    
    // Size-specific classes (using helper classes where possible)
    switch (this.size) {
      case 'small':
    ...
    classes.push(`fish-button--${this.size}`); // For padding and height
    
    // Variant-specific classes (colors will use helper classes in HTML)
  ...
  get colorClasses(): string {
    ...
    // Use helper classes for colors based on variant
    switch (this.variant) {
      case 'primary':
      ...
      case 'info':
        ...
        break;
      case 'ghost':
        // Ghost uses derived text color from SCSS
        break;
    }
